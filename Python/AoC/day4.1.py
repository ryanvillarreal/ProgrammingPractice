...
def organize_data(data):
	...
	for line in data.split('\n'):
		for info in line.split('\n'):
			print regex.findall(info)

def record_guards(guard_id):
	if guard_id in guards:
		print "Guard is already recorded."
	else:
		guards[guard_id] = 0

def calculate_time(time):
	print "time is just a construct, man"
...
def read_data(data):
	...
	for line in data.split('\n'):
		for info in line.split('\n'):
			print info
			if "#" in info:
				guard_id = regex.findall(info)[0]
				current_guard = guard_id
				record_guards(guard_id)
			elif "wakes" in info:
				print "Found wake up"
			elif "falls" in info:
				print "Found sleepy time"
	print guards
